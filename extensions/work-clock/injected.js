(function() {
    ...
    function removeAllWorkClockOverlays() {
        document.querySelectorAll('#work-clock-overlay, #work-clock-loading').forEach((el) => el.remove());
    }

    // 기존 오버레이/모달 정리 후 새로 생성
    ...
    loadingDiv.style.cssText = 'background:#ffffff;padding:28px;border-radius:18px;box-shadow:0 18px 40px rgba(31,26,23,0.2);font-family:"Space Grotesk","IBM Plex Sans KR","Apple SD Gothic Neo",sans-serif;min-width:360px;max-width:520px;width:100%;border:1px solid rgba(255,122,59,0.2);';
        ...
        .catch(err => {
            ...
            loadingDiv.innerHTML = `
                <div style="color:#d94b0c">
                    ...
                    <button onclick="document.querySelectorAll('#work-clock-overlay, #work-clock-loading').forEach(el => el.remove())" style="margin-top:10px;padding:10px 20px;background:#d94b0c;color:white;border:none;border-radius:999px;cursor:pointer">
                        닫기
                    </button>
                </div>
            `;
